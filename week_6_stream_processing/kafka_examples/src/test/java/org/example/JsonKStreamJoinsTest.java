...
class JsonKStreamJoinsTest {
	...
	public void setup() {
		...
		ridesTopic = testDriver.createInputTopic(JsonKStreamJoins.INPUT_RIDE_TOPIC, Serdes.String().serializer(), CustomSerdes.getSerde(Ride.class).serializer());
	
		pickupLocationTopic = testDriver.createInputTopic(JsonKStreamJoins.INPUT_RIDE_LOCATION_TOPIC, Serdes.String().serializer(), CustomSerdes.getSerde(PickupLocation.class).serializer());
		
		outputTopic = testDriver.createOutputTopic(JsonKStreamJoins.OUTPUT_TOPIC, Serdes.String().deserializer(), CustomSerdes.getSerde(VendorInfo.class).deserializer());
	}
